/* ----------------------------------------------------------------------------------------------------------------------
   $Author: sj364 $
   $Date: 2023/07/24 19:40:58 $
   $Source: /biolincc_data/framingham/programs/RCS/pull_offspring.sas,v $
    
   Purpose: Create Phenotype visit file and events file for Framingham Offspring.
    
   Assumptions: Source datasets exist
    
   Outputs: /biolincc_data/framingham/analdata/pheno_fram_offspring.sas7bdat

   ---------------------------------------------------------------------------------------------------------------------
   Modification History
   ---------------------------------------------------------------------------------------------------------------------
   $Log: pull_offspring.sas,v $
   Revision 1.2  2023/07/24 19:40:58  js463
   Put IDTYPE=1 selection back in.

   Revision 1.1  2023/07/24 17:16:54  js463
   Initial revision
...
/********************************************/
data fhstroke_v6(keep=pid fh_stroke);
 set in1.ex1_6d_v1(where=(idtype=1));
...
proc sort data=fhstroke_v6(keep=pid fh_stroke);
 by pid;
run;
proc freq data=fhstroke_v6;
 tables fh_stroke;
...
/* High sodium intake */
/**********************/
data hsod_v6(keep=pid sodium);
 set in1.vr_ffreq_ex06_1_0506d(where=(idtype=1) keep=pid nut_sodium idtype);

 sodium = nut_sodium;

run;

proc sort data=hsod_v6(keep=pid sodium);
 by pid;
run;
...
data hsod_v8(keep=pid sodium);
 set in1.vr_ffreq_ex08_1_0615d_v1(where=(idtype=1) keep=pid nut_sodium idtype);

 sodium = nut_sodium;

run;

proc sort data=hsod_v8(keep=pid sodium);
 by pid;
run;

proc univariate data=hsod_v8;
 var sodium;
 title 'Offspring visit 8';
run;

/********************************************/
/* Valvular heart disease, Carotid Stenosis */
/********************************************/
data valvdis_v3(keep=pid valvdis carsten);
 set in1.ex1_3d_v1(where=(idtype=1) keep=pid idtype c339 c340 c341 c361 c362);
...
proc sort data=valvdis_v3(keep=pid valvdis carsten);
 by pid;
run;
proc freq data=valvdis_v3;
 tables valvdis carsten;
 title 'Offspring visit 3';
run;


data valvdis_v6(keep=pid valvdis carsten);
 set in1.ex1_6d_v1(where=(idtype=1) keep=pid idtype f617 f618 f619 f564 f565);
...
proc sort data=valvdis_v6(keep=pid valvdis carsten);
 by pid;
run;
proc freq data=valvdis_v6;
 tables valvdis carsten;
 title 'Offspring visit 6';
run;


data valvdis_v8(keep=pid valvdis carsten);
 set in1.e_exam_ex08_1_0005d(where=(idtype=1) keep=pid idtype h339 h340 h341 h298 h299);
...
proc sort data=valvdis_v8(keep=pid valvdis carsten);
 by pid;
run;
proc freq data=valvdis_v8;
 tables valvdis carsten;
 title 'Offspring visit 8';
run;
...
/* Income */
/**********/
data income_v3(keep=pid fam_income);
 set in1.q_psych_ex03_1_0167d(where=(idtype=1) keep=pid py125 idtype);
...
proc sort data=income_v3(keep=pid fam_income);
 by pid;
run;

proc freq data=income_v3;
 tables fam_income;
 title 'Offspring visit 3';
run;
...
data alco_v3(keep=pid alcohol);
 set in1.ex1_3d_v1(where=(idtype=1) keep=pid c452 idtype);
...
/* exam 6 */
data alco_v6(keep=pid alcohol);
 set in1.vr_ffreq_ex06_1_0506d(where=(idtype=1) keep=pid ffd114-ffd117 idtype);
...
/* exam 8 */
data alco_v8(keep=pid alcohol);
 set in1.vr_ffreq_ex08_1_0615d_v1(where=(idtype=1) keep=pid ffd114-ffd117 idtype);
...
/*************************/
/* exam 3 */
data fruveg_v3(keep=pid fruits vegetables);
 set in1.q_ffreq_ex03_1_0302d(where=(idtype=1) keep=pid idtype apple orange grapfrut peaches bananas strawber blackber melon watermel
...
/* exam 6 */
data fruveg_v6(keep=pid fruits vegetables);
 set in1.vr_ffreq_ex06_1_0506d(where=(idtype=1) keep=pid idtype ffd30-ffd35 ffd37 ffd39 ffd42-ffd44 ffd45 ffd50-ffd70);
...
/* exam 8 */
data fruveg_v8(keep=pid fruits vegetables);
 set in1.vr_ffreq_ex08_1_0615d_v1(where=(idtype=1) keep=pid idtype ffd30-ffd35 ffd37 ffd39 ffd42-ffd44 ffd45 ffd50-ffd70);
...
/**********************************/
/* exam 3 (exam 2) */
data pai_v3(keep=pid inactivity activity activity_alt);
 set in1.ex1_2d_v3(where=(idtype=1) keep=pid idtype b107 b108);
...
/* exam 6 (exam 5) */
data pai_v6(keep=pid inactivity activity activity_alt);
 set in1.act1_5d_v1(where=(idtype=1) keep=pid idtype eh_10d eh_10e);
...
/* exam 8 */
data pai_v8(keep=pid inactivity activity activity_alt);
 set in1.e_exam_ex08_1_0005d(where=(idtype=1) keep=pid idtype h483 h484);
...
/*
proc freq data=in1.pht006023_v4_vr_svstk_2018_a_126(where=(idtype=1));
 tables stroke strokedate;
run;
proc freq data=in1.pht006024_v4_vr_svstktia_2018_a_(where=(idtype=1));
 tables stroke_tia stroketiadate;
run;
endsas;
*/
/*
proc contents data=in1.pht006027_v1_vr_wkthru_ex09_1_10(where=(idtype=1));
run;
proc freq data=in1.pht006027_v1_vr_wkthru_ex09_1_10(where=(idtype=1));
 tables date1 date2 date3/missing;
run;

data oldevents;
 set in3.events_r_fhs_offspring;
run;

proc sort data=oldevents;
 by usubjid_o;
run;

proc print data=oldevents(obs=25);
run;
...
data censonly;
 ...
 if first.usubjid_o and last.usubjid_o;
run;
proc print data=censonly;
run;
endsas;
*/
...
data hrtdisease2(rename=(procdate=hrtdt));
 set in1.vr_cvdproc_2020_a_1313d(where=(idtype=1));
...
/********************************************************************************/
data survcvd(keep=pid base_cvd);
 length base_cvd $3;
 set in1.vr_survcvd_2019_a_1334d(where=(idtype=1));

 if cvd=1 and (.<cvddate<=0) then base_cvd="YES";
  else base_cvd="NO";
run;

proc sort data=survcvd(keep=pid base_cvd);
 by pid;
run;
proc freq data=survcvd;
 tables base_cvd;
run;

/*******************************************************/
data svstk(drop=idtype);
 set in1.vr_svstk_2019_a_1335d(where=(idtype=1) keep=pid idtype stroke strokedate);
run;
...
data svstktia(drop=idtype);
 set in1.vr_svstktia_2019_a_1336d(where=(idtype=1) keep=pid idtype stroke_tia stroketiadate);
run;
...
data survstk(keep=pid base_stroke);
...
data survdth(keep=pid death_ind censday);
 length death_ind $3;
 set in1.vr_survdth_2019_a_1337d(where=(idtype=1));
...
proc sort data=survdth(keep=pid death_ind censday);
 by pid;
run;
...
data diabstat;
 set in1.vr_diab_ex09_1_1002d(where=(idtype=1));

 keep pid curr_diab1-curr_diab9;

run;
/*
proc freq data=diabstat;
 tables curr_diab1-curr_diab9;
run;*/
...
data dmdat1(keep=pid curr_diab1 rename=(curr_diab1=diab)) 
     dmdat2(keep=pid curr_diab2 rename=(curr_diab2=diab))
     dmdat3(keep=pid curr_diab3 rename=(curr_diab3=diab))
     dmdat4(keep=pid curr_diab4 rename=(curr_diab4=diab))
     dmdat5(keep=pid curr_diab5 rename=(curr_diab5=diab))
     dmdat6(keep=pid curr_diab6 rename=(curr_diab6=diab))
     dmdat7(keep=pid curr_diab7 rename=(curr_diab7=diab))
     dmdat8(keep=pid curr_diab8 rename=(curr_diab8=diab))
     dmdat9(keep=pid curr_diab9 rename=(curr_diab9=diab));
...
data sex;  /* merge this with calcvars data set */
 set in1.vr_dates_2019_a_1175d(where=(idtype=1));
...
data calcvars;
 set in1.vr_wkthru_ex09_1_1001d(where=(idtype=1));

 date1=0;

 keep pid age1-age9 bmi1-bmi9 hgt1-hgt9 wgt1-wgt9 currsmk1-currsmk9 date1-date9 dlvh1-dlvh9
...
data calcvar1(keep= pid age1 sex sex_c bmi1 hgt1 wgt1 currsmk1 date1 hrx1 tc1 hdl1 trig1 bg1 dlvh1
     ...
     calcvar2(keep= pid age2 sex sex_c bmi2 hgt2 wgt2 currsmk2 date2 hrx2 tc2 hdl2 trig2 bg2 creat2 dlvh2 
     ...
     calcvar3(keep= pid age3 sex sex_c bmi3 hgt3 wgt3 currsmk3 date3 hrx3 tc3 hdl3 trig3 bg3 fasting_bg3 dlvh3 
     ...
     calcvar4(keep= pid age4 sex sex_c bmi4 hgt4 wgt4 currsmk4 date4 hrx4 tc4 hdl4 trig4 bg4 fasting_bg4 dlvh4 
     ...
     calcvar5(keep= pid age5 sex sex_c bmi5 hgt5 wgt5 currsmk5 date5 hrx5 tc5 hdl5 trig5 bg5 fasting_bg5 creat5 dlvh5 
     ...
     calcvar6(keep= pid age6 sex sex_c bmi6 hgt6 wgt6 currsmk6 date6 hrx6 tc6 hdl6 trig6 bg6 fasting_bg6 creat6 dlvh6 
     ...
     calcvar7(keep= pid age7 sex sex_c bmi7 hgt7 wgt7 currsmk7 date7 hrx7 tc7 hdl7 trig7 bg7 fasting_bg7 creat7 dlvh7 
     ...
     calcvar8(keep= pid age8 sex sex_c bmi8 hgt8 wgt8 currsmk8 date8 hrx8 tc8 hdl8 trig8 bg8 fasting_bg8 creat8 dlvh8 /*dmrx8*/ 
     ...
     calcvar9(keep= pid age9 sex sex_c bmi9 hgt9 wgt9 currsmk9 date9 hrx9 tc9 hdl9 trig9 bg9 fasting_bg9 creat9 dlvh9 /*dmrx9*/
...
/* Exam 1 - vitals, meds */
data exam1(keep=pid sysbp1 sysbp2 diabp1 diabp2 afib anycholmed nonstatin);
 set in1.ex1_1d_v3(where=(idtype=1) keep=pid idtype a55 a57 a56 a58 a80 a158);
...
proc freq data=exam1_all;
 tables base_cvd hxcvd hxmi hxhrtd;
 title 'Exam 1';
run;

*proc contents data=exam1_all;
*run;


/* Exam 2 - vitals, meds */
data exam2(keep=pid sysbp1 sysbp2 diabp1 diabp2 afib educlev anycholmed nonstatin insulin);
 set in1.ex1_2d_v3(where=(idtype=1) keep=pid idtype b24 b26 b25 b27 b43 b61 b68 b269);
...
proc freq data=exam2_all;
 tables hxcvd hxmi hxhrtd;
 title 'Exam 2';
run;

*proc contents data=exam2_all;
*run;


/* Exam 3 - vitals, meds */
data exam3(keep=pid sysbp1 sysbp2 diabp1 diabp2 afib anycholmed nonstatin insulin);
 set in1.ex1_3d_v1(where=(idtype=1) keep=pid idtype c184 c286 c185 c287 c26 c29 c302);
...
proc freq data=exam3_all;
 tables hxcvd hxmi hxhrtd;
 title 'Exam 3';
run;

*proc contents data=exam3_all;
*run;


/* Exam 4 - vitals, meds */
data exam4(keep=pid sysbp1 sysbp2 diabp1 diabp2 afib anycholmed nonstatin insulin);
 set in1.ex1_4d_v1(where=(idtype=1) keep=pid idtype d192 d288 d193 d289 d030 d035 d306);
...
proc freq data=exam4_all;
 tables hxcvd hxmi hxhrtd;
 title 'Exam 4';
run;

*proc contents data=exam4_all;
*run;


/* Exam 5 - vitals, meds */
data exam5(keep=pid sysbp1 sysbp2 diabp1 diabp2 afib anycholmed nonstatin statin insulin aspirin genhlth);
 set in1.ex1_5d_v1(where=(idtype=1) keep=pid idtype e485 e581 e486 e582 e247 e246 e249 e245 e248 e254 e218 e219 e589 e065);
...
proc freq data=exam5_all;
 tables hxcvd hxmi hxhrtd;
 title 'Exam 5';
run;

*proc contents data=exam5_all;
*run;


/* Exam 6 - vitals, meds */
data exam6(keep=pid sysbp1 sysbp2 diabp1 diabp2 afib anycholmed nonstatin statin insulin aspirin genhlth);
 set in1.ex1_6d_v1(where=(idtype=1) keep=pid idtype f476 f576 f477 f577 f211 f210 f213 f209 f212 f218 f181 f182 f584 f087);
...
proc freq data=exam6_all;
 tables hxcvd hxmi hxhrtd;
 title 'Exam 6';
run;

*proc contents data=exam6_all;
*run;


/* Exam 7 - vitals, meds */
data exam7(keep=pid sysbp1 sysbp2 diabp1 diabp2 afib anycholmed nonstatin statin insulin aspirin genhlth);
 set in1.ex1_7d_v2(where=(idtype=1) keep=pid idtype g271 g354 g272 g355 g043 g042 g045 g041 g044 g050 g038 g039 g362 g514);
...
proc freq data=exam7_all;
 tables hxcvd hxmi hxhrtd;
 title 'Exam 7';
run;

*proc contents data=exam7_all;
*run;


/* Exam 8 - vitals */
data exam8(keep=pid sysbp1 sysbp2 diabp1 diabp2 afib hxdiab aspirin genhlth2);
 set in1.e_exam_ex08_1_0005d(where=(idtype=1) keep=pid idtype h111 h233 h112 h234 h016 h011 h012 h309 h714);
...
data exam8_meds(drop=idtype atc_cod1-atc_cod4);
 ...
 set in1.vr_meds_ex08_1_0280d(where=(idtype=1) keep=pid idtype medname atc_cod1-atc_cod4);
...
proc sort data=exam8_meds nodupkey;
 by pid medtyp medname;
run;

            
data exam8_medpat(keep=pid cnt_statin cnt_nonstat cnt_anychol cnt_insul cnt_asp cnt_statnonstat statin_names nonstat_names insul_names statnonstat_names);
...
data exam8_all(drop=cvdt base_cvd);
 ...
 /* Create Diabetes status variable */
/*
 if bg>200 or fasting_bg>126 or dmrx=1
   then diab=1;

 else if (.<bg<126 and dmrx=0) or 
         (.<bg<126 and dmrx=. and hxdiab not in (1,2))
   then diab=0;

 else if bg=. and dmrx ne 1
   then diab=.;
*/
run;
...
/* Exam 9 - vitals */
data exam9(keep=pid sysbp1 sysbp2 diabp1 diabp2 afib hxdiab aspirin genhlth2);
 set in1.e_exam_ex09_1b_0844d(where=(idtype=1) keep=pid idtype j116 j255 j118 j257 j018 j011 j012 j364 j890);
...
data exam9_meds(drop=idtype atc_cod1-atc_cod4);
 length medtyp $20 medname $25;
 set in1.vr_meds_ex09_1b_0879d(where=(idtype=1) keep=pid idtype medname atc_cod1-atc_cod4);
...
data exam9_medpat(keep=pid cnt_statin cnt_nonstat cnt_anychol cnt_insul cnt_asp cnt_statnonstat statin_names nonstat_names insul_names statnonstat_names);
 length statin_names nonstat_names insul_names statnonstat_names $100;
 set exam9_meds;
 by pid medtyp;
 retain cnt_statin cnt_nonstat cnt_anychol cnt_insul cnt_asp cnt_statnonstat
...
data exam3_all;
 merge exam3_all(in=a)
       valvdis_v3(keep=pid valvdis carsten)
       income_v3(keep=pid fam_income)
       alco_v3(keep=pid alcohol)
       fruveg_v3(keep=pid fruits vegetables)
       pai_v3;
...
data exam6_all;
 merge exam6_all(in=a)
       fhstroke_v6(keep=pid fh_stroke)
       hsod_v6(keep=pid sodium)
       valvdis_v6(keep=pid valvdis carsten)
       alco_v6(keep=pid alcohol)
       fruveg_v6(keep=pid fruits vegetables)
       pai_v6;
...
data exam8_all;
 merge exam8_all(in=a)
       hsod_v8(keep=pid sodium)
       valvdis_v8(keep=pid valvdis carsten)
       alco_v8(keep=pid alcohol)
       fruveg_v8(keep=pid fruits vegetables)
       pai_v8;
...
proc freq data=pheno_fram_offspring;
 tables visit*(hxcvd hxmi hxhrtd lvh afib educlev genhlth genhlth2 inactivity activity activity_alt) / list missing;
 tables visit*sex_c*sex_n / list missing;
run;
...
proc export data=pheno_fram_offspring
            outfile="/biolincc_data/framingham/analdata/pheno_fram_offspring.csv"
            dbms=csv
            replace;
run;

ods rtf file="/biolincc_data/framingham/analdata/pheno_fram_offspring_contents.rtf";
...
proc freq data=pheno_fram_offspring;
 tables anycholmed aspirin insulin nonstatin statin statnonstat;
 by visit;
run;

proc univariate data=pheno_fram_offspring;
 var sysbp1 sysbp2 diabp1 diabp2;
 by visit;
run;
*/
endsas;

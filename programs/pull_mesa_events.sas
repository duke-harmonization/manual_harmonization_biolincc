/* ----------------------------------------------------------------------------------------------------------------------
   $Author: sj364 $
   $Date: 2023/07/26 20:22:27 $
   $Source: /dcri/sigmadata/stroke_prediction/biolincc_data/mesa/programs/RCS/pull_mesa_events.sas,v $
    
   Purpose: Create Phenotype CV events file for MESA.
    
   Assumptions: Source datasets exist
    
   Outputs: /dcri/sigmadata/stroke_prediction/biolincc_data/mesa/analdata/cv_events.sas7bdat
            /dcri/sigmadata/stroke_prediction/biolincc_data/mesa/analdata/cv_events.csv

   ---------------------------------------------------------------------------------------------------------------------
   Modification History
   ---------------------------------------------------------------------------------------------------------------------
   $Log: pull_mesa_events.sas,v $
   Revision 1.1  2023/07/26 20:22:27  sj364
   Initial revision
...
libname in1 '/biolincc_data/mesa/data/Primary/Events/CVD/Data' access=readonly;
...
data censoring;
 set in1.mesaevthr2015_drepos_20200330(keep=mesaid fuptt);
run;
...
/* Getting Events */
/********************************************************************************************************/
/* All events, except Atrial Fibrillation, are in the "mesaevthr2015_drepos_20200330" data set.      */
/* Atrial Fibrillation events are input from the data set "mesaevaffu10_drepos_2015416" near       */
...
data cvevents(keep=mesaid evtype event_name_o event_val_c event_t2_o);
 ...
 set in1.mesaevthr2015_drepos_20200330(drop=strkbis1 strkbis2 strkbis3);
...
* Getting AFIB Events;
data afibevt(keep=mesaid evtype event_name_o event_val_c event_t2_o);
   attrib event_name_o label="Original Event Name" length=$200;
   attrib event_t2_o label="Original Analysis - Time to Event (Days)";
   attrib event_val_c label = "Original Event Value of EVENT_NAME_O" length=$50;

 set in1.mesaevaffu10_drepos_2015416(drop=afibcd afiblbl);
...
proc export data=cvevents
            outfile="/biolincc_data/mesa/analdata/cv_events.csv"
            dbms=csv
            replace;
run;


ods rtf file="/biolincc_data/mesa/analdata/cv_events_contents.rtf";
